...
const MAX_FILENAME_LENGTH = 100; // cap sanitized filename length
const ALLOWED_MIME_TYPES = ["image/jpeg", "image/png", "application/pdf"];
const FORM_UPLOADS_BUCKET = "form-uploads";

type FieldType = "TEXT" | "EMAIL" | "URL" | "NUMBER" | "TEXTAREA" | "FILE" | "CHECKBOX";

// Note: in-memory rate limiter works on a single-instance deployment (Railway).
// On multi-instance setups it becomes 5*N/hr. Acceptable for MVP; replace with
// Redis-backed limiter before scaling horizontally.
// Note: x-forwarded-for is partially client-controlled — same tradeoff as the
// existing /api/onboarding/submit route. Railway's load balancer prepends the real
// IP; clients cannot forge position 0 after Railway's header rewrites.
...
export async function POST(
  ...
  try {
    ...
    for (const field of fileFieldMeta) {
      ...
      if (file && file.size > 0) {
        ...
        if (!ALLOWED_MIME_TYPES.includes(file.type)) {
          return NextResponse.json(
            { error: `File "${field.label}" has an unsupported type` },
            { status: 400 }
          );
        }
    ...
    // Empty strings from FormData are converted to undefined for optional fields
    // so Zod .optional() behaves correctly (empty string would fail .email()/.url()).
    ...
    for (const field of nonFileFields) {
      ...
      if (field.type === "NUMBER") {
        ...
        // Convert empty string to undefined for optional fields (EMAIL, URL, TEXT, TEXTAREA)
        // so Zod .optional() skips format checks on empty input
        validationData[field.fieldKey] =
          raw !== null && (field.required || raw !== "") ? (raw as string) : undefined;
      }
    }
    ...
    const email = emailField
      ? ((validationData[emailField.fieldKey] as string | undefined) ?? null) || null
      : null;
    ...
    // Track uploaded paths so we can clean up on partial failure.
    ...
    for (const [fieldKey, { file, buffer }] of Array.from(fileBuffers.entries())) {
      ...
      if (uploadError) {
        ...
        // Clean up: delete the submission and any already-uploaded files
      ...
      const {
        data: { publicUrl },
      } = getSupabaseAdmin().storage.from(FORM_UPLOADS_BUCKET).getPublicUrl(storagePath);

      fileValues.set(fieldKey, JSON.stringify({ url: publicUrl, mimeType: file.type, size: file.size }));
    }

    // Create all FieldResponse records in a single transaction
    ...
    console.error("[templates/submit] Unexpected error:", err);
